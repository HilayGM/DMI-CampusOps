...
export async function testCampusOps(baseUrl) {
  ...
  const authorizedComment = { action: 'comment', baseVersion: 1, text: 'Nota ficticia de tecnico' };
  await action('technician-1', authorizedComment, 'technician-comment-replay');
  const authorizedReplay = await action('technician-1', authorizedComment, 'technician-comment-replay', 200);
  assert.equal(authorizedReplay.duplicate, true);
  assert.equal(authorizedReplay.incident.payload.notes.length, 1);
  await action('technician-1', { ...authorizedComment, text: 'Nota ficticia distinta' }, 'technician-comment-replay', 409);

  await action('coordinator-1', { action: 'assign', baseVersion: 2, technicianId: 'technician-2' }, 'reassign-operation');
  const forbiddenReplay = await action('technician-1', authorizedComment, 'technician-comment-replay', 403);
  assert.equal(forbiddenReplay.code, 'forbidden');
  assert.equal(Object.hasOwn(forbiddenReplay, 'incident'), false);
  ...
  await action('technician-2', { action: 'start', baseVersion: 3 }, 'start-operation');
  await action('technician-2', { action: 'resolve', baseVersion: 4, diagnosis: 'Cable de prueba sustituido' }, 'resolve-operation');
  const close = { action: 'close', baseVersion: 5 };
  ...
  assert.equal(replay.incident.version, 6);
  ...
  await action('coordinator-1', { action: 'reopen', baseVersion: 6 }, 'reopen-operation');
  const evidence = { action: 'add_evidence', baseVersion: 7, evidenceId: 'synthetic-photo-1' };
  ...
  await action('reporter-1', { action: 'comment', baseVersion: 8, text: 'Nota ficticia' }, 'comment-operation');
